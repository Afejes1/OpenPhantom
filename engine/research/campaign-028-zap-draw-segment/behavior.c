...
static void run_case(int depth, unsigned int width_bits, unsigned int color, int alias, int null_material)
{
    ...
    op_zap_draw_segment(wanted_start, wanted_end, depth, width, color);
    CHECK(stage == 3);
    verify_state();
}
int main(void)
{
    ...
    static const unsigned int widths[6] = {0, 0x80000000U, 0x3f800000U, 0x3fc00000U, 0x7f800000U, 0x7fc00013U};
    ...
    for (d = 0; d < 7; ++d)
        for (w = 0; w < 6; ++w)
            for (c = 0; c < 4; ++c)
                for (a = 0; a < 2; ++a)
                    for (n = 0; n < 2; ++n)
                        run_case(depths[d], widths[w], colors[c], a, n);
