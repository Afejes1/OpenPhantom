    ...
    for (p = 0; p < 81; ++p)
        for (q = 0; q < 9; ++q)
            for (m = 0; m < 2; ++m)
            {
                ...
                mutation = m;
                post_value = words[q];
                op_debug_page = words[p % 9];
                op_debug_visible = words[(p / 9) % 9];
                snapshot();
                expected_visible = 1;
                expected_pulse = 0;
                expected_page = op_debug_page + 1u;
                taken = expected_page == 4;
                if (!taken && (int)expected_page > 4)
                    expected_page = 0;
                expected_calls = taken;
                op_debug_next_page();
                finish();
            }
