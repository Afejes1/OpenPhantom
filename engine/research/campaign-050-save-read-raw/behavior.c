...
int op_stream_read(void *data, unsigned int bytes, unsigned int count, void *stream)
{
    CHECK(stage == 0 && data == buffers[row].data && bytes == sizes[size_case] && count == 1 &&
          stream == expected_input);
    verify();
    change();
    ++stage;
        ...
        for (answer = 0; answer < 5; ++answer)
            for (size_case = 0; size_case < 5; ++size_case)
                for (stream_case = 0; stream_case < 3; ++stream_case)
                    ...
                    {
                        seed();
                        CHECK(op_save_read_raw(buffers[row].data, sizes[size_case]) == answers[answer]);
                        CHECK(stage == 1);
                        verify();
                    }
    printf("save_read_raw: %d checks, %d failures\n", checks, failures);
    return failures != 0;
}
