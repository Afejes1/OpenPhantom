...
static int origin_case;
static const long offsets[] = {LONG_MIN, -1, 0, 1, LONG_MAX};
static const int origins[] = {-7, 0, 1, 2, INT_MAX};
int op_stream_seek(void *stream, long offset, int origin)
{
    CHECK(stage == 0 && stream == expected_input && offset == offsets[size_case] && origin == origins[origin_case]);
    verify();
    change();
    ++stage;
            ...
            for (size_case = 0; size_case < 5; ++size_case)
                for (stream_case = 0; stream_case < 3; ++stream_case)
                    for (mutate = 0; mutate < 2; ++mutate)
                        for (origin_case = 0; origin_case < 5; ++origin_case)
                        {
                            seed();
                            CHECK(op_save_seek_restore(offsets[size_case], origins[origin_case]) == answers[answer]);
                            CHECK(stage == 1);
                            verify();
                        }
    printf("save_seek_restore: %d checks, %d failures\n", checks, failures);
    return failures != 0;
}
