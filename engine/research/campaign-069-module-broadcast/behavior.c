...
static int kind = 2, length, rotation, duplicates, mode, result_case, key_case, event_case;
static int order[5], plan[5], planned, calls, query;
static unsigned int event_word;
static const int keys[6] = {0, 1, -1, INT_MIN, INT_MAX, 73};
static const int results[5] = {0, 1, -1, INT_MIN, INT_MAX};
static const unsigned int words[8] = {0, 1, 3, 4, 6, 7, 0x80000000u, 0xffffffffu};
...
static int observe(int node, unsigned int event, unsigned int argument, unsigned int bits)
{
    ...
    if (node != plan[position])
        exit(2);
    verify();
    CHECK(event == event_word && argument == 0 && bits == 0);
    ++calls;
    if (position == 0 && mode)
    {
...
static void seed(void)
{
    ...
    for (i = 0; i < 5; ++i)
    {
        ...
        nodes[i].value.status.raw = words[(event_case + i) % 8];
    ...
    query = keys[key_case];
    event_word = words[event_case];
    kind = event_word == 4 || event_word == 6 ? 1 : 2;
    planned = calls = 0;
    if (query)
    {
        for (i = 0; i < length; ++i)
                ...
                for (key_case = 0; key_case < 6; ++key_case)
                    for (mode = 0; mode < 4; ++mode)
                        for (result_case = 0; result_case < 5; ++result_case)
                            for (event_case = 0; event_case < 8; ++event_case)
                            {
                                seed();
                                expected = query ? (planned ? results[result_case] : 1) : 0;
                                actual = op_save_lifecycle(query, event_word);
