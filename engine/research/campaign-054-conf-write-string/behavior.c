...
static const unsigned int string_lengths[] = {0, 1, 3, 8, 16, 17};
...
static unsigned int write0(int h, const void *p, unsigned int n)
{
    return io_common(h, p, n, 0);
}
static unsigned int write1(int h, const void *p, unsigned int n)
{
                ...
                for (count_case = 0; count_case < 6; ++count_case)
                    for (pointer_case = 0; pointer_case < 2; ++pointer_case)
                        for (answer_case = 0; answer_case < 4; ++answer_case)
                            ...
                            {
                                seed();
                                tables[0].value.write = write0;
                                tables[1].value.write = write1;
                                count = string_lengths[count_case];
                                payload[count] = 0;
                                ...
                                response = answer_case == 0   ? count
                                           ...
                                           : answer_case == 2 ? count - 1u
                                                              : 0x80000000u;
                                enabled = values[handle_case] != 0 && pointer_case != 0;
                                if (!enabled)
                                    op_conf_services = 0;
                                snapshot();
                                expected = enabled && response == count;
                                CHECK(op_conf_write_string((const char *)argument) == expected);
                                CHECK(stage == enabled);
                                verify();
                            }
