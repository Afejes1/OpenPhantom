...
int main(void)
{
    ...
    for (p = 0; p < 64; ++p)
        for (n = 0; n < 20; ++n)
            ...
            {
                seed(p);
                op_kernel_dialog_count = n < 17 ? n : words[n - 13];
                snapshot();
                want = expected_dialog_count < 16;
                if (want)
                {
                    expected_dialogs[expected_dialog_count] = words[v];
                    expected_dialog_count++;
                }
                CHECK(op_kernel_add_dialog(words[v]) == want);
                verify();
                CHECK(calls == 0);
            }
