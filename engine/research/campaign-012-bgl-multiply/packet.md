# Prepared target: bgl_multiply

Team not done bp/bgl.c. Full unpadded span0x0041F8F8..0x0041F910: 24bytes, 2 operands. Every instruction, PE relocation and call boundary verified against owned original before source work. Pinned VC5 C /Od /MT.

void(const OP_MATRIX *rhs). multiply(CURRENT,rhs) with borrowed pointers; no copies/guards. Test distinct and same owned matrices, callback changes global/current; no restoration.
