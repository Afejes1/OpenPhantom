...
int main(void)
{
    ...
    for (p = 0; p < 4; ++p)
        for (mask = 0; mask < 16; ++mask)
            for (ni = 0; ni < 5; ++ni)
                for (m = 0; m < 3; ++m)
                    for (headers = 0; headers < 2; ++headers)
                    {
                        seed(p);
                        kind = 2;
                        mutation = m;
                        ...
                        expected_calls = release_count + (headers ? 4 : 0);
                        if (kind == 2 && release_count == 0)
                            expected_count = 0;
                        op_debug_reset(headers ? 0x80000000u : 0);
                        CHECK(upsert_done == (headers ? 4 : 0));
                        finish();
                    }
    printf("debug_reset: %d checks, %d failures\n", checks, failures);
    return failures != 0;
}
