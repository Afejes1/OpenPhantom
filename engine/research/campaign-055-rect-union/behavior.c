...
int main(void)
{
    ...
    for (profile = 0; profile < 2400; ++profile)
        for (alias = 0; alias < 2; ++alias)
        {
            ...
            top = a.y < b.y ? a.y : b.y;
            ar = wrapped((__int64)a.x + a.width);
            ab = wrapped((__int64)a.y + a.height);
            br = wrapped((__int64)b.x + b.width);
            bb = wrapped((__int64)b.y + b.height);
            want.x = left;
            want.y = top;
            want.width = ar < br ? wrapped((__int64)br - left) : wrapped((__int64)a.width + a.x - left);
            want.height = ab < bb ? wrapped((__int64)bb - top) : wrapped((__int64)a.height + a.y - top);
            expected_rectangles[0].value = want;
            op_rect_union(&rectangles[0].value, &rectangles[alias ? 0 : 1].value);
            verify();
        }
