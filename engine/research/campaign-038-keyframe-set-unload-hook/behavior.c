    ...
    for (old = 0; old < 3; ++old)
        for (new = 0; new < 3; ++new)
            for (repeat = 0; repeat < 2; ++repeat)
            {
                ...
                op_keyframe_load_hook = (repeat ? load_a : load_b);
                CHECK(op_keyframe_set_unload_hook(choices[new]) == choices[old]);
                CHECK(op_keyframe_unload_hook == choices[new]);
                CHECK(op_keyframe_load_hook == (repeat ? load_a : load_b));
                if (repeat)
                {
                    CHECK(op_keyframe_set_unload_hook(choices[new]) == choices[new]);
                    CHECK(op_keyframe_unload_hook == choices[new]);
                }
                CHECK(invoked == 0);
                verify();
