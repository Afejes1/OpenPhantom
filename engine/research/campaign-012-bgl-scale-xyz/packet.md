# Prepared target: bgl_scale_xyz

Team not done bp/bgl.c. Full unpadded span0x0041F895..0x0041F8D7: 66bytes, 3 operands. Every instruction, PE relocation and call boundary verified against owned original before source work. Pinned VC5 C /Od /MT.

void(float x,float y,float z). Copy external48-byte identity matrix into local, replace diagonal float indices0,4,8, multiply(CURRENT,&local). Test full synthetic external48-byte pattern with ONLY diagonal replaced, float bit preservation and callback pointer/state mutations; no zero/one gate. Standard memcpy intrinsic allowed if ordinary copy needs compiler hint.
