# Prepared target: bgl_rotate_axis

Team not done bp/bgl.c. Full unpadded span0x0041F79C..0x0041F7DF: 67bytes, 4 operands. Every instruction, PE relocation and call boundary verified against owned original before source work. Pinned VC5 C /Od /MT.

void(const OP_VEC3 *axis,float angle). Local48-byte matrix; if angle compares equal to 0 return; original pinned x87 also skips unordered NaN. Otherwise build_rotation(&local,axis,angle), then multiply(CURRENT,&local). Current is loaded after builder callback. Test +0/-0/quietNaN skip with null axis/current; signed finite and infinities forwarded unchanged to stub; builder fills all48 bytes and swaps current, multiply observes new current and same local. No invented finite guards.
