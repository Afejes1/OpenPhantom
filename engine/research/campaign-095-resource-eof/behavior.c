...
static int selected_operation = 1;
...
static void assertion(int table, const char *message, const char *source, int line)
{
    ...
    CHECK(message == op_resource_invalid_access && source == op_resource_source && line == 0x83c);
    verify();
    ++assert_calls;
    if (error_mutation)
    {
...
int main(void)
{
                            ...
                            for (error_mutation = 0; error_mutation < 2; ++error_mutation)
                            {
                                ...
                                expected_return = !should_active ? 1
                                                  ...
                                                  : selected_operation == 1       ? values[4 - v]
                                                                 : values[v];
                                result = op_resource_eof(selected);
    ...
    printf("resource_eof: %d checks, %d failures\n", checks, failures);
    return failures != 0;
}
