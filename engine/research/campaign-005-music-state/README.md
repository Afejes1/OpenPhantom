# Requested music state

The independently authored C implementation matches the **complete 26-byte
original interval**, with **2 verified address operands and zero differing
bytes** after address resolution. All **8 focused assertions pass**.
...
Focused getter evidence; returned requested state is not proof of actual playback state.
...
```powershell
python engine/research/campaign-001/verify.py --case engine/research/campaign-005-music-state --runner native --reference C:/Dev/Ghidra/projects/TPM/WMAIN.EXE
python engine/research/campaign-001/verify.py --case engine/research/campaign-005-music-state --runner docker --reference C:/Dev/Ghidra/projects/TPM/WMAIN.EXE --behavior --expect-report <native-report-path> --native-listing-fallback
```
