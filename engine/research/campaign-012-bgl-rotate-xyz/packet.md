# Prepared target: bgl_rotate_xyz

Team not done bp/bgl.c. Full unpadded span0x0041F7DF..0x0041F80B: 44bytes, 1 operands. Every instruction, PE relocation and call boundary verified against owned original before source work. Pinned VC5 C /Od /MT.

void(float x,float y,float z,float angle). Capture XYZ into local vector then call rotate_axis(&local,angle), including zero angle. Test exact float bits, order, local independent storage, callback changes, no post-write.
