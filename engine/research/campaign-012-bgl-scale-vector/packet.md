# Prepared target: bgl_scale_vector

Team not done bp/bgl.c. Full unpadded span0x0041F8D7..0x0041F8F8: 33bytes, 1 operands. Every instruction, PE relocation and call boundary verified against owned original before source work. Pinned VC5 C /Od /MT.

void(const OP_VEC3 *input). Forward input XYZ values to scale_xyz(x,y,z), no math/gates. Test all float bits and complete capture before callback changes input.
