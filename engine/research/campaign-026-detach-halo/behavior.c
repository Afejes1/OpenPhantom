...
void op_release_sprite(void **sprite)
{
    ...
    while (slot < 32 && expected[slot].owner != wanted)
        ++slot;
    CHECK(slot < 32);
    if (slot >= 32)
        return;
    ...
    *sprite = calls % 2 ? 0 : owned[slot];
    ...
    if (dynamic && slot == 0)
    {
        op_halos[1].owner = &actors[1].value;
        expected[1].owner = &actors[1].value;
...
static void run_case(int profile, int single)
{
    ...
    for (i = 0; i < 32; ++i)
    {
        op_halos[i].owner = &actors[1].value;
