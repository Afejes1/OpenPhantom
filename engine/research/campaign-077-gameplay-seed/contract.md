...
Complete original span 0x00470D56 to 0x00470DA1, 75 bytes, no tail. Shared signed RNG state is4B7950. Cdecl signatures are int next(void), int seed(int), int roll(int count,unsigned sides). All relevant operands are explicit in target.json.
