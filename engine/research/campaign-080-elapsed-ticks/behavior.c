...
int main(void)
{
        ...
        for (b = 0; b < 12; ++b)
            for (c = 0; c < 12; ++c)
                for (mutate = 0; mutate < 2; ++mutate)
                {
                    ...
                    calls = 0;
                    expected = (unsigned int)((unsigned __int64)returned + 0x100000000ui64 - after_start);
                    prior_after = after_prior;
                    result = op_elapsed_ticks();
    ...
    printf("elapsed_ticks: %d checks, %d failures\n", checks, failures);
    return failures != 0;
}
