...
{
    unsigned int unknown_00[6];
    void(__cdecl *assert_failure)(const char *, const char *, int);
    unsigned int unknown_1c[5];
    ...
    int(__cdecl *eof)(int);
    int(__cdecl *tell)(int);
    unsigned int unknown_4c[2];
    int(__cdecl *debug_print)(int, const char *, ...);
} OP_CONF_SERVICES;
typedef char
    conf_service_offsets[(sizeof(OP_CONF_SERVICES) == 0x58 && offsetof(OP_CONF_SERVICES, open) == 0x30 &&
                          offsetof(OP_CONF_SERVICES, close) == 0x34 && offsetof(OP_CONF_SERVICES, read) == 0x38 &&
                          offsetof(OP_CONF_SERVICES, write) == 0x40 &&
                          offsetof(OP_CONF_SERVICES, assert_failure) == 0x18 &&
                          offsetof(OP_CONF_SERVICES, eof) == 0x44 && offsetof(OP_CONF_SERVICES, tell) == 0x48 &&
                          offsetof(OP_CONF_SERVICES, debug_print) == 0x54)
                             ? 1
                             : -1];
