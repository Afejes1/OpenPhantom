# Prepared target: bgl_rotate_point

Team not done bp/bgl.c. Full unpadded span0x0041F942..0x0041F976: 52bytes, 2 operands. Every instruction, PE relocation and call boundary verified against owned original before source work. Pinned VC5 C /Od /MT.

void(OP_VEC3 *point). Local vector; rotate_basis(&local,point,CURRENT); copy3 local float words back to point after callback. Test distinct temporary, borrowed point/current snapshot, explicit outputs incl signedzero/quietNaN, callback mutates point/current, full neighboring state. Natural aggregate copy, no fake frames.
