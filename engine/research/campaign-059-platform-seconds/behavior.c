...
static const unsigned int time_seconds[] = {0x00000000u, 0x3a83126fu, 0x3be56042u, 0x3f800000u,
            ...
            for (r = 0; r < 3; ++r)
                for (time_index = 0; time_index < 7; ++time_index)
                    for (vector_index = 0; vector_index < 9; ++vector_index)
                    {
                        ...
                        mode = (int)r - 1;
                        seed();
                        CHECK(float_bits(op_platform_seconds()) ==
                              (initial_flag ? vectors[vector_index].seconds : time_seconds[time_index]));
                        CHECK(calls == 1);
                        verify();
