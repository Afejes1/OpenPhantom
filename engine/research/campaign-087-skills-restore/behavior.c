...
#include "save_read_support.c"
#include "save_read_raw_support.c"
            ...
            for (f = 0; f < 7; ++f)
                for (p = 0; p < 4; ++p)
                    for (mutate = 0; mutate < 2; ++mutate)
                    {
                        ...
                        stage = 0;
                        expected_result = context != 2 || file_result == 0;
                        expected_stage = context == 2 ? 1 : 0;
                        result = op_skills_restore(context);
