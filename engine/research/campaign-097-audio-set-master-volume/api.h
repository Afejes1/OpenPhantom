...
typedef struct OP_AUDIO_DEVICE_VT
{
    ...
    unsigned int(__stdcall *release)(OP_AUDIO_DEVICE *);
    unsigned int unknown0c[2];
    int(__stdcall *duplicate)(OP_AUDIO_DEVICE *, OP_AUDIO_BUFFER *, OP_AUDIO_BUFFER **);
    int(__stdcall *cooperative)(OP_AUDIO_DEVICE *, unsigned int, unsigned int);
} OP_AUDIO_DEVICE_VT;
typedef struct OP_AUDIO_BUFFER_VT
{
    int(__stdcall *query)(OP_AUDIO_BUFFER *, const void *, OP_AUDIO_SPATIAL **);
    ...
    int(__stdcall *volume)(OP_AUDIO_BUFFER *, int);
...
{
    unsigned int unknown00[13];
    int(__stdcall *orientation)(OP_AUDIO_LISTENER *, float, float, float, float, float, float, unsigned int);
    int(__stdcall *position)(OP_AUDIO_LISTENER *, float, float, float, unsigned int);
    unsigned int unknown3c[2];
...
typedef char audio_layout
    [(sizeof(void *) == 4 && sizeof(OP_AUDIO_VECTOR) == 12 && offsetof(OP_AUDIO_DEVICE_VT, cooperative) == 0x18 &&
      ...
      offsetof(OP_AUDIO_SPATIAL_VT, velocity) == 0x50 && offsetof(OP_AUDIO_LISTENER_VT, commit) == 0x44)
         ? 1
         : -1];
/* Frozen32-bit storage name: contains the device pointer at00862074. */
extern unsigned int op_audio_gate_word;
extern OP_AUDIO_BUFFER *op_audio_primary;
extern int op_audio_primary_volume;
extern const unsigned char op_audio_spatial_iid[16];
...
typedef char audio_lifecycle_layout
    [(offsetof(OP_AUDIO_DEVICE_VT, release) == 8 && offsetof(OP_AUDIO_DEVICE_VT, duplicate) == 0x14 &&
      offsetof(OP_AUDIO_BUFFER_VT, query) == 0 && offsetof(OP_AUDIO_BUFFER_VT, volume) == 0x3c &&
      offsetof(OP_AUDIO_LISTENER_VT, orientation) == 0x34 && offsetof(OP_AUDIO_LISTENER_VT, position) == 0x38)
         ? 1
         : -1];
OP_AUDIO_SPATIAL *op_audio_get_spatial(OP_AUDIO_BUFFER *);
OP_AUDIO_BUFFER *op_audio_duplicate(OP_AUDIO_BUFFER *);
void op_audio_shutdown(void);
void op_audio_set_listener(const OP_AUDIO_VECTOR *, const OP_AUDIO_VECTOR *, const OP_AUDIO_VECTOR *);
void op_audio_set_master_volume(float);
#endif
