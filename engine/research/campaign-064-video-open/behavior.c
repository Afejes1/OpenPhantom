                ...
                for (allocate_ok = 0; allocate_ok < 2; ++allocate_ok)
                    for (open_ok = 0; open_ok < 2; ++open_ok)
                        for (change_state = 0; change_state < 2; ++change_state)
                        {
                            seed(p);
                            mutate = change_state;
                            op_video_sound_driver = initial ? &tokens[initial - 1][0] : 0;
                            driver_result = get_result ? &tokens[get_result - 1][0] : 0;
                            allocation_result = allocate_ok ? active_video : 0;
                            handle_result = open_ok ? &tokens[2][0] : 0;
                            snapshot();
                            if (!initial)
                            {
                                codes[expected_count++] = 1;
                                if (get_result)
                                    codes[expected_count++] = 2;
                            }
                            codes[expected_count++] = 3;
                            if (allocate_ok)
                            {
                                codes[expected_count++] = 4;
                                codes[expected_count++] = open_ok ? 5 : 6;
                            }
                            result = op_video_open(active_name, active_description, argument3_value, argument4_value);
                            CHECK(result == (allocate_ok && open_ok ? active_video : 0));
                            finish();
                        }
