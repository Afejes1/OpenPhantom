# Prepared target: bgl_transform_xyz

Team not done bp/bgl.c. Full unpadded span0x0041F9D6..0x0041FA37: 97bytes, 1 operands. Every instruction, PE relocation and call boundary verified against owned original before source work. Pinned VC5 C /Od /MT.

void(float *x,float *y,float *z_optional). Capture *x,*y and z?*z:0.0f into local; transform_point(&local); then write x, y, then if z write z. Test null and present z, +0 null input, local distinct and all captured before callback mutates originals, aliased outputs last write wins (all same=>Z; X/Y same and null Z=>Y). X/Y required valid, no extra guards.
