        ...
        for (null_input = 0; null_input < 2; ++null_input)
            for (has_handle = 0; has_handle < 2; ++has_handle)
                for (change_state = 0; change_state < 2; ++change_state)
                {
                    seed(p);
                    mutate = change_state;
                    active_video->handle = has_handle ? &tokens[1][0] : 0;
                    captured_handle = active_video->handle;
                    snapshot();
                    if (!null_input)
                    {
                        if (has_handle)
                            codes[expected_count++] = 7;
                        codes[expected_count++] = 6;
                    }
                    CHECK(op_video_close(null_input ? 0 : active_video) == (!null_input && has_handle));
                    finish();
                }
