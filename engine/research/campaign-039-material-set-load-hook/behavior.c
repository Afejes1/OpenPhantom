    ...
    for (old = 0; old < 3; ++old)
        for (new = 0; new < 3; ++new)
            for (repeat = 0; repeat < 2; ++repeat)
            {
                ...
                op_material_unload_hook = repeat ? unload_a : unload_b;
                CHECK(op_material_set_load_hook(choices[new]) == choices[old]);
                CHECK(op_material_load_hook == choices[new]);
                CHECK(op_material_unload_hook == (repeat ? unload_a : unload_b));
                if (repeat)
                {
                    CHECK(op_material_set_load_hook(choices[new]) == choices[new]);
                    CHECK(op_material_load_hook == choices[new]);
                }
                CHECK(invoked == 0);
            }
