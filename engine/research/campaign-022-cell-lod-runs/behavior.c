...
static void verify(const unsigned char *runs, unsigned int count)
{
    ...
    expected = state;
    for (i = 0; i < count; ++i)
        expected.cell.records[i].lod_run = runs[i];
    op_cell_lod_runs(&state.cell);
...
int main(void)
{
    ...
    for (i = 0; i < 255; ++i)
    {
        state.cell.records[i].lod_mask = 255;
    ...
    for (i = 0; i < 128; ++i)
    {
        state.cell.records[i].lod_mask = 128;
        runs[i] = (unsigned char)(127 - i);
    }
    verify(runs, 128);
    seed(3);
    state.cell.records[0].lod_mask = 1;
    state.cell.records[1].lod_mask = 3;
    state.cell.records[2].lod_mask = 1;
    {
        static const unsigned char containment[3] = {2, 0, 0};
        verify(containment, 3);
    }
    seed(3);
    ...
    state.cell.records[2].lod_mask = 255;
    {
        static const unsigned char zero_mask[3] = {2, 1, 0};
        verify(zero_mask, 3);
    }
    seed(3);
    state.cell.records[1].lod_mask = 128;
    state.cell.records[1].flags = 0x8000;
    verify(zeros, 3);
    printf("cell LOD runs: %d checks, %d failures\n", checks, failures);
    return failures != 0;
}
