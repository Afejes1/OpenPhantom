# Save cached music IDs

The independently authored C implementation matches the **complete 61-byte
original interval**, with **6 verified address operands and zero differing
bytes** after address resolution. All **21 focused assertions pass**.
...
The worker used 2 candidate compiler invocation(s). The coordinator
...
This saves only two requested IDs; no actual file I/O, voice state or device persistence is tested.
...
```powershell
python engine/research/campaign-001/verify.py --case engine/research/campaign-005-music-save --runner native --reference C:/Dev/Ghidra/projects/TPM/WMAIN.EXE
python engine/research/campaign-001/verify.py --case engine/research/campaign-005-music-save --runner docker --reference C:/Dev/Ghidra/projects/TPM/WMAIN.EXE --behavior --expect-report <native-report-path> --native-listing-fallback
```
