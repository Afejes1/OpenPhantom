...
static int __stdcall callback0(OP_AUDIO_BUFFER *object)
{
    observe(object, 0);
    return return_code;
}
static int __stdcall callback1(OP_AUDIO_BUFFER *object)
{
    observe(object, 1);
    return return_code;
}
static void seed(int expected_calls)
{
    ...
    tables[0].stop = callback0;
    tables[1].stop = callback1;
    for (i = 0; i < 2; ++i)
                ...
                for (ri = 0; ri < 5; ++ri)
                    for (a = 0; a < 1; ++a)
                        for (p = 0; p < 2; ++p)
                        {
                            ...
                            if (initial_gate && p)
                                continue;
                            expected_calls = initial_gate != 0;
                            arg_word = 0;
                            arg_data = 0;
                            seed(expected_calls);
                            wanted_this = p ? 0 : &buffers[selected].value;
                            result = op_audio_pause(wanted_this);
                            CHECK(result == (initial_gate != 0));
