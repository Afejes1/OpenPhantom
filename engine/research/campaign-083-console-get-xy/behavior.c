...
static OP_CONSOLE_COORD *destination(int index, int expected)
{
    switch (index)
    {
    case 0:
        return expected ? &expected_owned[1].coord : &owned[1].coord;
    case 1:
        return expected ? &expected_output.coord : &op_console_output.coord;
    case 2:
        return expected ? &expected_input.coord : &op_console_input.coord;
    case 3:
        return expected ? &expected_default.coord : &op_console_default_attr.coord;
    default:
        return expected ? &expected_current.coord : &op_console_current_attr.coord;
    }
}
int main(void)
{
            ...
            for (r = 0; r < 5; ++r)
                for (p = 0; p < 5; ++p)
                    for (mutate = 0; mutate < 2; ++mutate)
                    {
                        ...
                        wanted_code = 2;
                        captured_handle = op_console_output.bits;
                        op_console_get_xy(destination(p, 0));
                        destination(p, 1)->x = returned_position.x;
                        destination(p, 1)->y = returned_position.y;
                        CHECK(calls == 1);
                        verify();
