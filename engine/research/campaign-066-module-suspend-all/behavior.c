...
static int resume = 0, length, rotation, duplicates, mode, result_case, key_case, status_case;
...
static const unsigned int status_words[8] = {0,           8,           0x80000000u, 0x80000008u,
...
{
    return resume ? (word & 8) != 0 : (word & 8) == 0;
}
static int observe(int node, unsigned int event, unsigned int argument, unsigned int bits)
{
    ...
    {
        if (resume)
            expected_nodes[node].value.status.raw &= ~8u;
        else
            expected_nodes[node].value.status.raw |= 8u;
    }
    return returned;
...
static void seed(void)
{
    ...
    {
        for (i = 0; i < length; ++i)
            visits[i] = order[resume ? length - 1 - i : i];
        for (i = 0; i < length; ++i)
                ...
                for (key_case = 0; key_case < 6; ++key_case)
                    for (mode = 0; mode < 4; ++mode)
                        for (result_case = 0; result_case < 5; ++result_case)
                            ...
                            {
                                seed();
                                expected = query ? (planned ? returns[result_case] : resume) : 0;
                                actual = op_module_suspend_all(query);
