# Prepared target: bgl_translate_vector

Team not done bp/bgl.c. Full unpadded span0x0041F80B..0x0041F837: 44bytes, 3 operands. Every instruction, PE relocation and call boundary verified against owned original before source work. Pinned VC5 C /Od /MT.

void(const OP_VEC3 *input). Local matrix; build_translation(&local,input) then multiply(CURRENT,&local). Builder receives original borrowed pointer; can swap current and fills all48 bytes. Multiplication must use newly loaded current and same complete local. No null guards.
