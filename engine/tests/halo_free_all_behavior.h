...
static int lc_halo_free_all_expected_count, lc_halo_free_all_calls, lc_halo_free_all_last_slot,
    lc_halo_free_all_dynamic;
...
void lc_halo_free_all_op_release_sprite(void **sprite)
{
    int lc_halo_free_all_slot = lc_halo_free_all_last_slot + 1;
    while (lc_halo_free_all_slot < 32 && lc_halo_free_all_expected[lc_halo_free_all_slot].owner == 0)
        ++lc_halo_free_all_slot;
    lc_halo_free_all_CHECK(lc_halo_free_all_slot < 32);
    if (lc_halo_free_all_slot >= 32)
        return;
    lc_halo_free_all_CHECK(sprite == &op_halos[lc_halo_free_all_slot].sprite);
    lc_halo_free_all_expected[lc_halo_free_all_slot].owner = 0; /* Required clear before callback, not after it. */
    lc_halo_free_all_verify_state();
    ++lc_halo_free_all_calls;
    lc_halo_free_all_last_slot = lc_halo_free_all_slot;
    ...
    *sprite = lc_halo_free_all_calls % 2 ? 0 : lc_halo_free_all_owned[lc_halo_free_all_slot];
    lc_halo_free_all_expected[lc_halo_free_all_slot].sprite = *sprite;
...
{
    int lc_halo_free_all_i, lc_halo_free_all_total = 0;
    ...
    lc_halo_free_all_last_slot = -1;
    op_halo_free_all();
    lc_halo_free_all_expected_count = 0;
    lc_halo_free_all_CHECK(lc_halo_free_all_calls == lc_halo_free_all_total);
    lc_halo_free_all_verify_state();
}
